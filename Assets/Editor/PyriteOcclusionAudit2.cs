// Tools ▸ Pyrite2 ▸ Z35a. Occlusion Audit (Ctrl+Alt+Shift+5) / Z35b. Occlusion Measure (Ctrl+Alt+Shift+4) / Z35c. Occlusion Flags + Bake (Ctrl+Alt+Shift+3)
//  (옛 M0/M5 를 Pyrite2 로 옮김 — Pyrite 메뉴가 화면을 넘어 눌리지 않는다)
...
public static class PyriteOcclusionAudit2
{
    const string LOG = "Logs/pyrite_occlusion.txt";
    static readonly string[] M_SKIP = { "Mirror", "TimePanel", "TimeDial", "LightFX", "Ambience", "Main Camera", "VRCWorld", "EventSystem", "LakeMirror", "LakeMirrorSwitch", "SkyDiscs", "SpinChair", "MediaPlayer" };

    [MenuItem("Tools/Pyrite2/Z35a. Occlusion Audit %&#5", false, 90)]
    public static void Audit()
    {
        ...
        sb.AppendLine("RESULT: AUDIT"); Directory.CreateDirectory("Logs"); File.AppendAllText(LOG, sb.ToString());
    }
    ...
    [MenuItem("Tools/Pyrite2/Z35b. Occlusion Measure %&#4", false, 91)]
    ...
    static void Finish()
    {
        ...
        msb.AppendLine("RESULT: MEASURED"); Directory.CreateDirectory("Logs"); File.AppendAllText(LOG, msb.ToString());
        Debug.Log("[M5] 완료 — Logs/pyrite_occlusion.txt");
    }

    // 플래그 정리(M) → 백그라운드 굽기 → 끝나면 저장·용량 기록
    static double bakeT0;
    [MenuItem("Tools/Pyrite2/Z35c. Occlusion Flags + Bake %&#3", false, 92)]
    public static void FlagsAndBake()
    {
        if (StaticOcclusionCulling.isRunning) { File.AppendAllText(LOG, "[Z35c] 이미 굽는 중\n"); return; }
        float before = StaticOcclusionCulling.umbraDataSize / 1048576f;
        PyriteOcclusionTools.SetupFlags();
        StaticOcclusionCulling.GenerateInBackground();
        bakeT0 = EditorApplication.timeSinceStartup;
        File.AppendAllText(LOG, string.Format("[Z35c] {0} 플래그 정리 + 굽기 시작 (이전 데이터 {1:0.00} MB)\n", System.DateTime.Now.ToString("HH:mm:ss"), before));
        EditorApplication.update -= BakeTick; EditorApplication.update += BakeTick;
    }

    static void BakeTick()
    {
        if (StaticOcclusionCulling.isRunning) return;
        EditorApplication.update -= BakeTick;
        UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty(SceneManager.GetActiveScene());
        UnityEditor.SceneManagement.EditorSceneManager.SaveOpenScenes();
        File.AppendAllText(LOG, string.Format("[Z35c] {0} 굽기 끝 {1:0} s, 데이터 {2:0.00} MB\nRESULT: BAKED\n", System.DateTime.Now.ToString("HH:mm:ss"), EditorApplication.timeSinceStartup - bakeT0, StaticOcclusionCulling.umbraDataSize / 1048576f));
    }
